...
import { CellSelection, TableMap, cellAround } from '@remirror/pm/tables'
...
import type { Events } from './jsx'
import { cellSelectionToSelection } from './prosemirror'
import type { CellAxis, FindTable } from './types'
...
export function injectControllers({
  ...
  const headerControllerCells: ProsemirrorNode[] = repeat(
    controllerCell,
    getMap().width + 1,
  )

  const controllerRow: ProsemirrorNode = schema.nodes.tableRow!.create(
    {},
    headerControllerCells,
  )
