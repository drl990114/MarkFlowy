...
import type { CellAxis, FindTable } from '../utils/types'
import type { InsertButtonAttrs } from './table-insert-button'
  ...
  const trigger = h('div', {
    ...
    onMouseEnter: (event) => {
      ...
      if (axis) {
        ...
        if (axis.row > 0) {
          type = isTopLeft
            ? TriggerAreaType.ADD_ROW_BEFORE
            : TriggerAreaType.ADD_ROW_AFTER
        }
        else if (axis.col > 0) {
          type = isTopLeft
            ? TriggerAreaType.ADD_COLUMN_BEFORE
            : TriggerAreaType.ADD_COLUMN_AFTER
        }
...
function TableInsertButtonTrigger({
  view,
  findTable,
}: {
  view: EditorView
