...
type InsertLinkOrImageOptions = CommandKeymapOptions & { type: string }
function insertLinkOrImage(target: EditorView, options: InsertLinkOrImageOptions) {
  const pre = options.type === 'image' ? '!' : ''
...
export function getInsertLinkOrImageCommand({
  type,
  options,
}: {
  type: string
  ...
  if (type === 'link') {
    return (target: EditorView) => insertLinkOrImage(target, { ...options, type })
  } else {
    return (target: EditorView) => insertLinkOrImage(target, { ...options, type })
  }
}
