...
describe('Command Palette global keybinding', () => {
  let root: Root
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    commandRegistry.registerCommand({ id: EVENT.app_commandPalette, handler: open })
  ...
  afterEach(() => {
    act(() => root.unmount())
    commandRegistry.unregisterCommand(EVENT.app_commandPalette)
  ...
  async function mount(keyMap: string[]) {
    vi.mocked(invoke).mockResolvedValue({
      ...
      rules: getDefaultKeybindings('linux')
        .filter((rule) => rule.command === EVENT.app_commandPalette)
  ...
  it.each([
    ...
    'opens once before a nested editor consumes $modifier+Shift+P and prevents printing',
    async ({ modifier, event }) => {
      await mount([modifier, 'Shift', 'p'])
      const nested = vi.fn((nestedEvent: KeyboardEvent) => nestedEvent.stopPropagation())
      target.addEventListener('keydown', nested)
      expect(press({ key: 'p', shiftKey: true, ...event }).defaultPrevented).toBe(true)
  ...
  it('honors a customized or disabled shortcut', async () => {
    await mount(['Alt', 'o'])
    expect(press({ key: 'p', shiftKey: true, ctrlKey: true }).defaultPrevented).toBe(false)
  ...
  it('ignores IME and other dialogs but captures the key within Command Palette', async () => {
    await mount(['Ctrl', 'Shift', 'p'])
    press({ key: 'p', shiftKey: true, ctrlKey: true, isComposing: true })
    press({ key: 'p', shiftKey: true, ctrlKey: true, repeat: true })
    press({ key: 'p', shiftKey: true, ctrlKey: true, keyCode: 229 })
    ...
    dialog.setAttribute('data-mf-command-palette', '')
    expect(press({ key: 'p', shiftKey: true, ctrlKey: true }).defaultPrevented).toBe(true)
    expect(open).toHaveBeenCalledOnce()
  })
})
