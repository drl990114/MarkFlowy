...
const useBookMarksStore = create<BookMarksStore>()(
  persist(
        ...
        toggleTag: (tag) =>
          set((state) => ({
            expandedTags: state.expandedTags.includes(tag)
              ? state.expandedTags.filter((item) => item !== tag)
    ...
    {
      name: 'mf:desktop:bookmarks-view',
      version: 1,
      storage: jsonStateStorage(),
      partialize: ({ viewMode, expandedTags }) => ({ viewMode, expandedTags }),
      merge: (saved, current) => {
        const value = isRecord(saved) ? saved : {}
        return {
          ...current,
          viewMode: value.viewMode === 'tags' ? 'tags' : 'list',
          expandedTags: Array.isArray(value.expandedTags)
            ? value.expandedTags.filter((tag): tag is string => typeof tag === 'string')
            : [],
        }
      },
    },
...
interface BookMarksStore {
  viewMode: 'list' | 'tags'
  expandedTags: string[]
  setViewMode: (mode: 'list' | 'tags') => void
