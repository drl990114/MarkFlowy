  ...
  return (
    ...
    >
      ...
      <CommandPrimitive.Input
        className={cn(
          'h-8 w-full bg-transparent text-ui-control outline-none placeholder:text-muted-foreground disabled:opacity-50',
          className,
        )}
...
export function CommandList({ className, ...props }: ComponentProps<typeof CommandPrimitive.List>) {
  return (
    <CommandPrimitive.List
      className={cn('max-h-56 scroll-py-1 overflow-x-hidden overflow-y-auto overscroll-contain p-1', className)}
...
export function CommandItem({ className, ...props }: ComponentProps<typeof CommandPrimitive.Item>) {
  return (
    <CommandPrimitive.Item
      className={cn(
        'relative flex min-h-7 cursor-default select-none items-center gap-2 rounded-sm px-2 py-1 text-ui-control outline-none data-[disabled=true]:pointer-events-none data-[disabled=true]:text-disabled-foreground data-[selected=true]:bg-control-selected data-[selected=true]:text-foreground',
        className,
      )}
