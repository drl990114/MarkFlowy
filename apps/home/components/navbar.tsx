...
export function Navbar({ links, ...props }: NavbarProps) {
  return (
    <header className="w-full top-0 z-50 flex-shrink-0 py-4 bg-white fixed" {...props}>
      <div className="container flex flex-col items-start justify-between px-6 mx-auto md:flex-row md:items-center">
        ...
        <div className="absolute flex justify-end md:static top-2 right-4"><LocaleSwitcher /></div>
      </div>
    </header>
  )
}
...
function MenuLink({ link }: { link: ILink }) {
  const cls = classNames('py-4 hover:underline text-sm md:text-base cussor-pointer')
  return (
    <li>
      {link.type === 'contorl' ? (
